    ...
    <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-gray-500">{{ $product->sku }} - {{ $product->name }}</p>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="unit" value="{{ old('unit', $product->unit ?? 'ชิ้น') }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="cost" value="{{ old('cost', $product->cost) }}" required min="0" step="0.01"
                ...
                <div>
                    ...
                    <div class="relative">
                        <span class="absolute left-3 top-2 text-gray-500">฿</span>
                        <input type="number" name="price_retail" value="{{ old('price_retail', $product->retail_price) }}" required min="0" step="0.01"
                            class="w-full pl-8 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        <span class="absolute left-3 top-2 text-gray-500">฿</span>
                        <input type="number" name="price_wholesale" value="{{ old('price_wholesale', $product->wholesale_price) }}" min="0" step="0.01"
                            class="w-full pl-8 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        <span class="absolute left-3 top-2 text-gray-500">฿</span>
                        <input type="number" name="price_technician" value="{{ old('price_technician', $product->vip_price) }}" min="0" step="0.01"
                            class="w-full pl-8 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        <span class="absolute left-3 top-2 text-gray-500">฿</span>
                        <input type="number" name="price_online" value="{{ old('price_online', $product->partner_price) }}" min="0" step="0.01"
                            class="w-full pl-8 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="reorder_point" value="{{ old('reorder_point', $product->branchStocks->first()->reorder_point ?? 5) }}" min="0"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            <div class="flex items-start gap-6">
                ...
                <div class="flex-shrink-0">
                    <p class="text-sm text-gray-500 mb-2">รูปปัจจุบัน:</p>
                    <img src="{{ Storage::url($product->image) }}" class="h-32 w-32 object-cover rounded-lg">
                </div>
                @endif

                <div class="flex-1" x-data="{ imagePreview: null }">
                    <input type="file" name="image" accept="image/*"
                        @change="imagePreview = URL.createObjectURL($event.target.files[0])"
                        class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100">
                    <p class="text-xs text-gray-500 mt-1">รองรับ JPG, PNG, GIF ขนาดไม่เกิน 2MB (อัปโหลดใหม่เพื่อเปลี่ยนรูป)</p>

                    <div x-show="imagePreview" class="mt-4">
                        <p class="text-sm text-gray-500 mb-2">รูปใหม่:</p>
