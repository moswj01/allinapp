...
<head>
    ...
    <style>
        ...
        html, body { width: {{ $pageW }}; }
        .panel { margin-bottom: {{ $panelGap }}; }
    </style>
</head>
<body>
    ...
    <div class="panel">
        <div class="header">
            <div>
                ...
                <div class="sub" style="margin-top:6px;">ชื่อผู้รับ: {{ $repair->customer_name }} • เบอร์ติดต่อ: {{ $repair->customer_phone }} • วันที่พิมพ์: {{ now()->format('d/m/Y') }}</div>
            </div>
            <div class="meta">
        ...
        <div class="section white card">
            <div class="grid">
                <div>
                    <div class="row"><div class="label">อุปกรณ์</div><div class="value">{{ $repair->device_brand }} {{ $repair->device_model }}</div></div>
                    <div class="row"><div class="label">ประเภท</div><div class="value">{{ $repair->device_type }}</div></div>
                    <div class="row"><div class="label">สี</div><div class="value">{{ $repair->device_color ?: '-' }}</div></div>
                    <div class="row"><div class="label">หมายเลข SN/IMEI</div><div class="value">{{ $repair->device_serial ?: ($repair->device_imei ?: '-') }}</div></div>
                    <div class="row"><div class="label">รหัสปลดล็อค</div><div class="value">{{ $repair->device_password ?: '-' }}</div></div>
                </div>
                <div>
                    <div class="row"><div class="label">วันที่รับ</div><div class="value">{{ optional($repair->received_at)->format('d/m/Y') ?? $repair->created_at->format('d/m/Y') }}</div></div>
                    <div class="row"><div class="label">ราคาประเมินซ่อม</div><div class="value">฿ {{ number_format($repair->estimated_cost ?? 0, 0) }}</div></div>
                    <div class="row"><div class="label">มัดจำ</div><div class="value">฿ {{ number_format($repair->deposit ?? 0, 0) }}</div></div>
                    <div class="row"><div class="label">อาการเสีย</div><div class="value">{{ $repair->problem_description }}</div></div>
                </div>
            </div>
        </div>

        @php
            $acc = is_array($repair->device_accessories) ? implode(', ', $repair->device_accessories) : ($repair->device_accessories ?? '');
        @endphp
        @if(!empty($acc))
        <div class="section white card">
            <div class="section-title">อุปกรณ์ที่ติดมาด้วย</div>
            <div class="row"><div class="label">รายการ</div><div class="value">{{ $acc }}</div></div>
        </div>
        @endif
        ...
        <div class="grid" style="margin-top:10px; align-items:start;">
            ...
            <div style="text-align:right;">
                <img class="qr" src="https://api.qrserver.com/v1/create-qr-code/?size=110x110&data={{ urlencode($trackUrl) }}" alt="QR Track" style="display:inline-block; width:110px; height:110px; border:1px solid var(--line); border-radius:8px; background:#fff;">
