...
@section('page-title', 'เพิ่มสาขาใหม่')
        ...
        <form action="{{ route('branches.store') }}" method="POST">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="line_token" value="{{ old('line_token') }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                    <p class="text-xs text-gray-500 mt-1">สำหรับส่งการแจ้งเตือนผ่าน LINE</p>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">หัวใบเสร็จ</label>
                    <textarea name="receipt_header" rows="2"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">{{ old('receipt_header') }}</textarea>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">ท้ายใบเสร็จ</label>
                    <textarea name="receipt_footer" rows="2"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">{{ old('receipt_footer') }}</textarea>
                </div>
