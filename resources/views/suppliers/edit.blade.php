...
@section('title', 'แก้ไขซัพพลายเออร์ - ' . $supplier->name)
@section('page-title', 'แก้ไขซัพพลายเออร์')
...
<div class="max-w-3xl mx-auto">
    <form action="{{ route('suppliers.update', $supplier) }}" method="POST" class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-gray-900">แก้ไขซัพพลายเออร์</h2>
                <p class="text-gray-500">{{ $supplier->code }} - {{ $supplier->name }}</p>
            </div>
            <a href="{{ route('suppliers.show', $supplier) }}" class="text-indigo-600 hover:text-indigo-800">
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="credit_days" value="{{ old('credit_days', $supplier->credit_days) }}" min="0"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500"
                        placeholder="30">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="address" rows="2"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500"
                        placeholder="ที่อยู่">{{ old('address', $supplier->address) }}</textarea>
                </div>
